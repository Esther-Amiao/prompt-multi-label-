...
# real_answer_dir = '../data_codereview/data/GraphCodeBert_data/dev1_set.csv'
...
def answer_engineering(tag_prediction):
    ...
    '''
    1. 去掉里边的[PAD]
    2. 依次读每个答案TOP5的每一个预测tag，然后3个[MASK]位置，依次读出来，看在不在整个tag_list里
        1）分个如果[MASK1]在：直接保存这个结果
    '''
    # 1
    ...
    for i in range(len(tag_prediction)):
        ...
        for j in range(len(tag_prediction[i])):
            ...
            # 设置一个状态记录，如果从3做到1都没有在tag_list里的，就设置状态变为0
            state = 1
            list_ij = tag_ij.split(" ")

            # 先判断这个在不在tag_list里边（这个做的是 排除：去掉PAD后只有一个词 和 三个词就是一个tag）
